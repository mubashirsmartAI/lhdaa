        ...
        <div class="show-div setting">
            ...
            <ul>
                @foreach($languageList as $key => $listl)
                    @php
                        $language = $listl->language ?? null;
                    @endphp
                    @if($language)
                        <li class="{{$applocale ==  $language->sort_code ?  'active' : ''}}">
                            <a href="javascript:void(0)" class="customerLang" langId="{{$listl->language_id}}">{{$language->name}}
                                @if($language->id != 1)
                                ({{$language->nativeName}})
                            @endif
                         </a>
