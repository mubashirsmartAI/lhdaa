...
@section('content')
            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">{{ __('Edit Beneficiary') }}</h4>
                    </div>
                    <div class="card-body">
                        <form method="POST" action="{{ route('beneficiary.update', $beneficiary->id) }}"
                            ...
                            <div class="mb-3">
                                ...
                                <input type="tel" class="form-control @error('phone') is-invalid @enderror" id="phone"
                                    placeholder="Your phone number" value="{{ old('phone', $beneficiary->phone ?? '') }}">

                                <input type="hidden" name="dial_code" id="dial_code" value="{{ old('dial_code', $beneficiary->dial_code ?? '') }}">
                                <input type="hidden" name="phone" id="phone_number" value="{{ old('phone', $beneficiary->phone ?? '') }}">
                            ...
                            <div class="form-group">
                                ...
                                <textarea name="description" id="description" class="form-control @error('desc') is-invalid @enderror" rows="4">{{ old('description', $beneficiary->description) }}</textarea>
                                @error('description')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-3">
                                ...
                                <input type="file" accept="image/*" data-plugins="dropify" name="image"
                                    class="dropify @error('image') is-invalid @enderror"
                                    data-default-file="{{ $beneficiary->image }}" />
                                @error('image')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
    ...
    $(document).ready(function () {
        ...
        let savedDialCode = "{{ old('dial_code', $beneficiary->dial_code ?? '') }}".replace('+', '');
        let savedPhone = "{{ old('phone', $beneficiary->phone ?? '') }}";
